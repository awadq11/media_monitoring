...
with st.sidebar:
    ...
    if st.button(
        ...
        with st.spinner(
            "جاري جلب وتصنيف أحدث أخبار الجوازات..."
        ):
...
if not df.empty:
    ...
    st.markdown(
        """
        <p class="red-search-label">
            🔍 البحث السريع في النتائج المرصودة:
        </p>
        """,
        unsafe_allow_html=True
    )
...
if not df.empty:
    ...
    with st.expander(
        "📌 استعراض تفاصيل وخيارات خبر محدد"
    ):
        ...
        st.markdown(
            f"""
            **المصدر:**
            `{matched_row["مصدر الخبر"]}`

            | **التاريخ:**
            `{matched_row["تاريخ النشر"]}`
            """
        )
        ...
        st.markdown(
            f"""
            [🔗 رابط المصدر]({matched_row["رابط الخبر"]})
            """
        )
